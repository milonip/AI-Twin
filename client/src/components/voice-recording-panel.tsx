...
import { Mic, Square, Play, RotateCcw, Loader2, Type, Send } from "lucide-react";
...
export default function VoiceRecordingPanel({ onVoiceProcessed, isProcessing }: VoiceRecordingPanelProps) {
  ...
  const handleTextSubmit = () => {
    if (textInput.trim()) {
      onVoiceProcessed(textInput.trim());
      setTextInput("");
    }
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="space-y-4">
          <div className="flex space-x-2">
            <Input
              ...
              placeholder="Type your message to chat with your AI Twin..."
      ...
      {/* Processing Status - shown for both modes */}
      {(isProcessing || recordingState === 'processing') && (
        <div className="text-center mt-4">
          <div className="inline-flex items-center space-x-2 px-4 py-2 bg-yellow-50 border border-yellow-200 rounded-full">
            ...
            <span className="text-sm text-yellow-600">Analyzing your message...</span>
          </div>
        </div>
      )}
    </div>
